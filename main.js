...
async function ensureModelosDocumentosTable() {
  await db.query(`
    CREATE TABLE IF NOT EXISTS modelos_documentos (
      ...
      config_cuadricula TEXT,
      es_predeterminado BOOLEAN DEFAULT FALSE,
      fecha_subida TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )
  `);
  try {
    await db.query(`ALTER TABLE modelos_documentos ADD COLUMN IF NOT EXISTS config_cuadricula TEXT;`);
  } catch (e) {}
}
...
ipcMain.handle('db:get-modelos-documentos', async (event, tipo) => {
  try {
    ...
    let queryStr = 'SELECT id, nombre, tipo, descripcion, archivo_nombre, archivo_tipo, archivo_data, config_cuadricula, COALESCE(es_predeterminado, FALSE) AS es_predeterminado, fecha_subida FROM modelos_documentos';
    const params = {};
    if (tipo && tipo !== 'Todos') {
    ...
    if (modelo.id) {
      await db.query(`
        ...
        SET nombre = @nombre, tipo = @tipo, descripcion = @descripcion,
            ...
            config_cuadricula = COALESCE(@config_cuadricula, config_cuadricula),
      ...
      `, {
        ...
        config_cuadricula: typeof modelo.config_cuadricula === 'object' ? JSON.stringify(modelo.config_cuadricula) : (modelo.config_cuadricula || null),
      ...
      const result = await db.query(`
        INSERT INTO modelos_documentos (nombre, tipo, descripcion, archivo_nombre, archivo_tipo, archivo_data, config_cuadricula, es_predeterminado)
        VALUES (@nombre, @tipo, @descripcion, @archivo_nombre, @archivo_tipo, @archivo_data, @config_cuadricula, @es_predeterminado)
        RETURNING id
      `, {
        nombre: modelo.nombre || 'Modelo sin título',
        tipo: docTipo,
